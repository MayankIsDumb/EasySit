...
/**
 * Use this class to manage sit/lay entities correctly
 */
public class SitUtil {
	...
	/**
	 * <dimension type id, <position, <entity, previous player position>>> Lay entities tracking. Server side only.
	 */
	private static final Map<Identifier, Map<BlockPos, Pair<LayEntity, Vec3>>> LAY_OCCUPIED = new HashMap<>();
	//?} else {
	/*private static final Map<ResourceLocation, Map<BlockPos, Pair<SitEntity, Vec3>>> OCCUPIED = new HashMap<>();
	private static final Map<ResourceLocation, Map<BlockPos, Pair<LayEntity, Vec3>>> LAY_OCCUPIED = new HashMap<>();
	*///?}
	...
	public static boolean addLayEntity(Level level, BlockPos blockPos, LayEntity entity, Vec3 playerPos) {
		if (!level.isClientSide()) {
			//? if >=1.21.11 {
			Identifier id = getDimensionTypeId(level);
			//?} else {
			...
			*///?}
			...
			LAY_OCCUPIED.computeIfAbsent(id, unused -> new HashMap<>());
			LAY_OCCUPIED.get(id).put(blockPos, Pair.of(entity, playerPos));
			return true;
		}
	...
	public static boolean removeLayEntity(Level level, BlockPos pos) {
		if (!level.isClientSide()) {
			//? if >=1.21.11 {
			Identifier id = getDimensionTypeId(level);
			//?} else {
			...
			*///?}
			...
			if (LAY_OCCUPIED.containsKey(id)) {
	...
	public static LayEntity getLayEntity(Level level, BlockPos pos) {
		if (!level.isClientSide()) {
			//? if >=1.21.11 {
			Identifier id = getDimensionTypeId(level);
			//?} else {
			...
			*///?}
			...
			if (LAY_OCCUPIED.containsKey(id) && LAY_OCCUPIED.get(id).containsKey(pos))
				return LAY_OCCUPIED.get(id).get(pos).getLeft();
		}

		return null;
	}

	public static Vec3 getPreviousLayPosition(Player player, LayEntity layEntity) {
		if (!player.level().isClientSide()) {
			//? if >=1.21.11 {
			Identifier id = getDimensionTypeId(player.level());
			//?} else {
			...
			*///?}
			...
			if (LAY_OCCUPIED.containsKey(id)) {
				for (Pair<LayEntity, Vec3> pair : LAY_OCCUPIED.get(id).values()) {
					if (pair.getLeft() == layEntity)
						return pair.getRight();
				}
			}
		}
	...
	public static boolean isLayOccupied(Level level, BlockPos pos) {
		//? if >=1.21.11 {
		Identifier id = getDimensionTypeId(level);
		//?} else {
		/*ResourceLocation id = getDimensionTypeId(level);
		*///?}
		...
		return LAY_OCCUPIED.containsKey(id) && LAY_OCCUPIED.get(id).containsKey(pos);
	}

	public static boolean isPlayerLaying(Player player) {
		for (var entry : LAY_OCCUPIED.entrySet()) {
			for (Pair<LayEntity, Vec3> pair : entry.getValue().values()) {
				if (pair.getLeft().hasPassenger(player))
					return true;
			}
		}
